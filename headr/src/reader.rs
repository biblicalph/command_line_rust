...
pub fn read_lines(filename: &str, reader: &mut impl BufRead, num_lines: i64) -> Vec<String> {
  ...
  if num_lines > 0 {
      for _ in 0..num_lines {
          ...
          let is_eof = reader.read_line(&mut buf).expect("should read line from reader") == 0;
          if is_eof {
              break;
          }
...
pub fn read_bytes(
  ...
  if num_bytes > 0 {
      buf = reader.bytes().take(num_bytes as usize).collect::<Result<Vec<_>, _>>()?;
  } else {
      ...
      for elem in reader.bytes() {
          if ring_buffer.len() == num_bytes {
              buf.push(ring_buffer.pop_front().expect("ring buffer element should exist"));
          }
